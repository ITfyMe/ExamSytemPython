...
class SellarPoeNotificationService(BaseNotificationService):
    ...
    def _build_description(self, action, data, template, conf):
        ...
        invoice_id = self.get_direct("sNotificationSellerBillGet", params_object, False)
        template = template.replace("<DOC_TYPE>", str(display_name))
        template = template.replace("<INV_NUM>", str(invoice_id[0]["Updated_Bill_Number"]))
        return template
