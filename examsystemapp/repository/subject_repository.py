...
class SubjectRepo(BaseRepository):
    ...
    def pre_update(self, object: SubjectModel):
        self.sp_name = "sSubjectUpdate"
        list_params = [object.subjectid,object.name,object.code]
        self.params_list = list_params
    ...
    def pre_get(self, params):
        self.sp_name = "sSubjectGet"
        self.params_list = params.get_params_list()

    def post_get(self, cursor_object):
        if len(cursor_object.get_data()) > 0:
            subject_model: SubjectModel = SubjectModel()
            for each_tuple in cursor_object.get_data():
                subject_model.subjectid = each_tuple[0]
                subject_model.name = each_tuple[1]
                subject_model.code = each_tuple[2]
    ...
    def post_get_list(self, cursor_object):
        ...
        if len(cursor_object.get_data()) > 0:
            for each_tuple in cursor_object.get_data():
                ...
                list_data.append(subject_model)
    ...
    def pre_get_object(self, params):
        self.sp_name = "sSubjectObjectGet"
        self.params_list = params.get_params_list()
    ...
    def pre_get_list_object(self, params: ParamsObject):
        self.sp_name = "sSubjectObjectGetList"
        self.params_list = params.get_params_list()
